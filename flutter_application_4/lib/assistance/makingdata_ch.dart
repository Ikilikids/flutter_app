...
class OriginCentral2 {
  Map<String, String> ct;
  Map<String, dynamic> numberData;

  OriginCentral2({required this.ct, required this.numberData});
  ...
  Map<String, dynamic> makingvariable() {
    ...
    if (mainsort == "discriminant") {
      ...
      p3 = nn;
    } else if (mainsort.contains("inequality_t")) {
      int k = mainsort.contains("2") ? 2 : 1;
      String allmax = mainsort.contains("2") ? "2\\pi" : "180°";
      String range =
          mainsort.contains("1") ? "0<\\theta<180°とする;" : "0<\\theta<2\\piとする;";
      if (detailsort != "tan") {
        int q = randomint(1, 2);
        if (q == 1) {
          List<String> crList = [];
          ...
          while (crList.length < 4) {
            ...
            if (crList.isEmpty && detailsort == "easy" || p == 4 || p == 5) {
              if (p == 1 || p == 4) {
                if (c > 0) {
                  cr = "$gonly1<x<$gonly2";
                }
            ...
            } else if (crList.isEmpty && detailsort == "hard" || p == 3) {
              if (a > 0 && b > 0) {
...
// 最終的に crList に 4 つの重複なし cr が入っている
      ...
      q1 = "$range$q1の解は、";
    } else if (mainsort.contains("value_t")) {
...
// 最終的に crList に 4 つの重複なし cr が入っている
    ...
    } else if (mainsort.contains("inequality_ex")) {
      int a = randomint(2, 5);
      int b = randomint(2, 3);
      int k = pow(a, b).toInt();
      List<String> crList = [];
...
// 最終的に crList に 4 つの重複なし cr が入っている
      ...
      p2 = crList[2];
      p3 = crList[3];
    } else if (mainsort.contains("inequality_log")) {
      int a = randomint(2, 5);
      int b = randomint(2, 3);
      int k = pow(a, b).toInt();
      List<String> crList = [];
