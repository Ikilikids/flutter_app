...
class SoundpoolPlugin : MethodChannel.MethodCallHandler, FlutterPlugin {
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        ...
        with(application.cacheDir) { 
            this?.list { _, name -> name.matches("sound(.*)pool".toRegex()) }?.forEach { File(this, it).delete() } 
        }
    }
...
// 以下、SoundpoolWrapper や VolumeInfo はそのままコピー
...
internal class SoundpoolWrapper(private val context: Context, private val maxStreams: Int, private val streamType: Int) {
    ...
    private val loadingSoundsMap = HashMap<Int, MethodChannel.Result>()
    ...
    private fun createSoundpool() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        setOnLoadCompleteListener { _, sampleId, status ->
            ...
            resultCallback?.let {
                ui {
                    if (status == 0) it.success(sampleId) else it.error("Loading failed", "Error code: $status", null)
                }
                ...
                loadExecutor.execute {
                    try {
                        ...
                        FileOutputStream(tempFile).use {
                            ...
                            if (soundId > -1) loadingSoundsMap[soundId] = result else ui { result.success(soundId) }
                        }
                    } catch (t: Throwable) { ui { result.error("Loading failure", t.message, null) } }
                }
            }
            "play" -> {
                val arguments = call.arguments as Map<String, Any>
                ...
                val volumeInfo = volumeSettingsForSoundId(soundId)
                runBg {
                    val streamId = soundPool.play(soundId, volumeInfo.left, volumeInfo.right, 0, repeat, rate.toFloat())
                    ui { result.success(streamId) }
                }
            }
            // ここに pause, resume, stop, setVolume, setRate, loadUri なども元のコードをそのままコピー
